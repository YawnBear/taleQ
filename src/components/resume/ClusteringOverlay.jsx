...
export default function ClusteringOverlay({ 
    showOverlay, 
    setShowOverlay, 
    clusteringPrompt, 
    setClusteringPrompt, 
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            // Check if the overlay is open and the click is outside the overlay content
            if (showOverlay && overlayContentRef.current && !overlayContentRef.current.contains(event.target)) {
                closeOverlay();
            }
        };
            ...
            {showOverlay && (
                <div className="fixed inset-0 bg-white/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div 
                        ...
                        className="bg-white rounded-lg shadow-xl w-[1000px] h-[800px] flex flex-col"
                    >
                        {/* Header */}
                        <div className="px-6 py-4 border-b bg-gradient-to-r from-green-50 to-white flex justify-between items-center">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-semibold text-gray-800">
                                    {showResults ? 'Clustering Results' : 'Resume Clustering (AI-Powered)'}
                                </h2>
                            </div>
                            <button 
